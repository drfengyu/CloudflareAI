...
- **用户留存**：
  ...
  - 限时活动·幸运转盘（`/lottery`）：credits 购券开奖，内圈固定加减 cr / 外圈按倍数结算 cr 或直接赠抽奖券（界面只显实际 cr，单抽与 10 连分列），累抽满档位送券；中奖进临时余额、倒扣可扣成负余额，资金变动统一 `topup type=6` 流水（赠券不动 cr）；「我的活动记录」逐次可查，管理端 `/admin/lottery` 看用户参与与站点净收益
