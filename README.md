...
当前版本：**v0.3.0**（2026-06-18）—— AI 供应商渠道管理完整实现
...
**最新更新（v0.3.0）**：
- ✅ AI 供应商渠道管理：完整 CRUD + 适配器框架 + 健康检查 + 模型同步
- ✅ OpenAI/Anthropic 适配器实现（直通 + 健康检查 + 模型列表）
- ✅ 动态配置表单（根据渠道类型自动切换字段）
- ✅ 渠道详情页（关联密钥/模型列表/热门排行/统计）
- ✅ 修复 Drizzle leftJoin 兼容性（v0.45.2 无该导出）
