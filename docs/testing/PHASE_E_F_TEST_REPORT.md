...
- [x] 点击"批量生成"按钮
- [x] 对话框正常打开
- [x] 填写生成参数（数量=10, 额度=5, 使用次数=1, 有效期=30天）
- [x] 点击"生成"按钮
- [x] 生成成功，显示 10 个兑换码
...
**详细说明**:
- 初始余额：21,518 cr = $21,518.00 USD
- 兑换码：`3TH9SHQJR4VBCJNJ` (5 cr)
- **兑换后余额：21,523 cr = $21,523.00 USD** ✓
- 新增流水记录：
  - 类型：兑换码充值
  - 金额：+5 cr = $5.00
...
- 用户列表显示：
  ...
  - 角色：超级管理员
  - **余额：21,523 cr = $21,523.00**（与钱包页面一致）
  - 注册时间：3 天前
- 管理对话框功能：
  - 显示当前余额
  - 余额调整表单（金额输入框 + 说明）
...
- [x] 用户余额更新正确
- [x] 充值流水记录正确
- [x] 跨页面余额显示一致（钱包 21,523 cr = 用户管理 21,523 cr）
