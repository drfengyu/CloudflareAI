...
3. 转盘是**两段式指针**（暗色霓虹风）：
   - **内圈**：奖品格（加 / 减「券价 × 倍率」cr）+ **唯一一个「外圈入口」格**，入口占满 `outerChancePercent`（默认 12.5%）。扇区角度由 `innerSectorLayout` 与抽奖同源，格子大小就是真实概率。
...
内圈存的是 `multiplier`（可为负），实际 cr = `multiplier × ticketPriceCredits`（`innerPrizeCredits`）；
外圈本来就是倍数结算。**所以改单券价只会等比放大整池金额，返还率、亏损概率这些手感指标一动不动。**

早期内圈存绝对 cr，结果券价从 100 调到 125 时奖池一分没涨、成本涨了 25%，
单券返还率从 90.7% 悄悄掉到 75.1%——界面还显示同一套数字，没人看得出奖池变薄了。

旧配置（`innerPrizes[].credits`）读回来时按**当次券价**折成倍率（`sanitizeInner`），
所以线上那份绝对值奖池的金额一分不变，只是从下一次改券价开始会跟着缩放。
后台表单每行的「本档 cr」列与顶部三行期望（内圈 / 外圈 / 单券综合 + 返还率）都按当前输入实时重算。
...
```jsonc
{
  ...
  "prizeValidDays": 7,
  "innerPrizes": [{ "multiplier": 0.48, "weight": 25 }, ...],   // 旧写法 credits: 60 仍可读，按当次券价折算
  "outerPrizes": [
...
- 默认奖池配平到**单券返还率 ≈ 91.1%**，两圈都按券价倍率计价，所以这个数与券价无关。改奖池时以后台那行
  「内圈 / 外圈 / 单券综合期望 + 返还率」为准，超过 100% 即每卖一张券净亏。
- 时间一律北京时间墙钟串（`lib/date.ts` 的 `parseCnWallClock`），避免服务器 UTC 造成活动提前/延后开关。
- **一次买券上限 100 张**受 `MAX_TICKETS_PER_PURCHASE` 控制；发券走 `batchRows(values, 6)` 分片写入，
