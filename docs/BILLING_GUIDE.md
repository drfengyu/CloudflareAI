...
详见 `docs/features/lottery.md`。这里只记与账本相关的口径。

| 动作 | 资金流向 | 流水 |
|------|---------|------|
| 购买抽奖券 | 先扣临时余额（按过期时间升序），不足再扣永久余额；**总额不足则整笔失败**，不允许透支买券 | `topup type=6`，金额为负 |
| 中奖（正数） | 发放到**临时余额**，有效期 `prizeValidDays` 天 | `topup type=6`，金额为正 |
| 倒扣（负数） | 直接扣永久余额，**允许扣成负数**（与消费侧 `deductCredits` 同一口径） | `topup type=6`，金额为负 |
| 累抽赠券 | 不动余额，只多发行券 | 无流水（发券记录在 `lottery_ticket`） |

- 抽奖不产生 `usage_log`，因此**不计入看板的调用次数与模型消耗**，只影响余额与流水。
- 钱包「充值记录」里 type=6 的正数行按签到/兑换码同一套 `withoutExpiredGrants` 处理（过期即不再显示），
  负数行（买券、倒扣）是永久账目，永不隐藏。
- D1 经 REST 没有事务，所以扣款与锁券都走 `d1Run` 的条件更新，用返回的 `changes` 判断是否真的命中；
  单次开奖失败只撤销那一次（删开奖行 + 删流水 + 删临时余额行 / 反向补回永久余额 + 退券）。
