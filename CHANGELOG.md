...
- **限时活动·内圈奖池改为「倍率 × 券价」计价**（`lib/lottery/prize-math.ts`）：`InnerPrize.credits` 换成 `multiplier`，实际 cr 由 `innerPrizeCredits(config, prize) = 倍率 × 券价` 现算，与外圈同一口径。此前券价从 100 调到 125 时，绝对值奖池一分没涨而成本涨了 25%，单券返还率从 90.7% 悄悄掉到 75.1%，界面却毫无变化；现在整池随券价等比缩放，**返还率与定价解耦**（实测券价 100 / 125 / 150 / 999 下候选池返还率恒为 91.12%）。
  - 旧配置兼容：`sanitizeInner` 见到 `credits` 就按**当次券价**折成倍率，线上那份绝对值奖池读回来的金额一分不变（实测 60/130/200/−60/−200/400/1000/−400 逐档一致），只是从下一次改券价开始会跟着缩放。
  - 后台表单实时重算：顶部面板拆成「内圈期望 / 外圈期望 / 单券综合 + 返还率 + 站点每券净得」，券价、外圈概率、任一圈的倍率或权重一改就重算（实测 125→250 时两圈期望翻倍而返还率仍是 75.11%）；内圈每行补「本档 cr」只读列，外圈每行补「本档实际」（batch 基数下同时给单抽与 10 连两个数）。`updateLotterySettings` 的内圈校验随之改收倍率（±1000）。
