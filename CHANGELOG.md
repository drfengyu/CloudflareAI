...
- **限时活动·外圈赠券奖型**：外圈奖品从「只有 cr」扩成两种奖型（`OuterPrize.kind = "credits" | "tickets"`），抽中直接给 N 张抽奖券。
  ...
  - 默认外圈奖池扩到 **16 档**（12 档 cr + 4 档赠券）：内圈正档加厚、外圈倍数下调、负档收平（单券返还率 91.1%、单发亏损概率 19.6%，按 125 cr 券价算区间 −150 ~ +2500 cr）。**上线顺序固定为「代码先部署，再改 `option.lottery_config`」**——旧代码会把 `kind: "tickets"` 读成 `multiplier` 缺失，真实用户将抽到一片 0 cr 空档。

### 变更

- **10 连抽从「逐注串行写库」改成「整批算完再一次写」**（`lib/lottery/store.ts` + `app/(dashboard)/lottery/actions.ts`，详见 `docs/features/lottery.md`）：
  - 一次 10 连抽原先要打约 40 次**串行** D1 HTTP 往返（锁券 10 次 + 每注各一次开奖行/流水/余额变动），单程 0.4~0.9 秒，实测 7~10 秒。现在整批结果先在内存里 `planDrawBatch` 算完，写入按表合并成一条语句（`lockTickets` 用一条 `CASE` 锁完整批、`insertDrawRows`/`insertTopupRows`/`insertTemporaryBalanceRows` 走多行 INSERT、倒扣合并成一次净额 UPDATE），互不相关的语句并发发出：**往返数 40+ → 9，关键路径 4 波，实测一次 10 连抽 2.4 秒**。
  - 锁券那条语句把「有一张被占用就一张都不锁」写进 SQL 本身（末尾 `(SELECT COUNT(*) ... WHERE usedDrawId IS NULL) = 张数`），而不是锁完再比较命中数不够才回滚——回滚是第二条语句，中间那一瞬并发请求能把刚锁上的券再偷走，用户白掉券。
  - **回滚粒度由「单次开奖」变成「整批撤销」**：任一写入失败就删开奖行、删流水、删临时余额行、反向补回净额、收回发出的券、解锁券，不再有「10 注里留下 3 注」的半截状态。
  - 档位赠券不再吞掉所有异常：按报错文本认出 `UNIQUE constraint failed` 当作「这一档早领过」跳过，其它异常照旧抛出并拖垮整批（静默吞异常正是当初「送多张的档位全部失效」的病灶）。`grantMilestoneTickets`/`settleDraw`/`undoSettlement` 随之下线。
  - 券包余量改按本批收支算出（少一次写入后的 COUNT 往返）。
  - 验证：真实 10 连抽账本逐字段对账通过（10 注 seq 连续、券↔开奖双向无孤儿、`topup` 合计 = −1250 + 1955、9 笔中奖全额进临时余额、余额按「先扣临时再扣永久」闭合）；故障注入下一次单抽返回「开奖失败，本次未消耗抽奖券」，回滚后账本快照与注入前逐字段完全一致。
...
- **窄扇区补记号，标签不再凭空消失**（`app/(dashboard)/lottery/lottery-wheel.tsx`）：上一条「放不下就不画」会让 1% 上下的格子变成空白 wedge，看不出那格里有奖品。现在退化成单个记号——**★ = cr 档、券 = 赠券档**（`markerSize()`，弧长够放 6px 才画，<2° 才真的留白）。同时把内圈标签的落点半径从 `INNER_R × 0.63` 提到 `× 0.76`：同一扇区角在更大半径处弧长更多，实测 5.3% 概率的「+400 cr」从"被判放不下"恢复成正常显示，外圈两条 ★ 分别落在 ×10 与 ×−5 档上（Chromium 截图核对）。
- **限时活动·奖池面板实时重算，券价明确为独立利润率杠杆**（`lib/lottery/prize-math.ts` + `lottery-form.tsx`）：后台顶部那块指标从单行「单券期望返还 / 返还率」拆成**内圈期望 / 外圈期望 / 单券综合 / 返还率 / 站点每券净得**，券价、外圈概率、任一圈的数值或权重一改就重算（`lotteryExpectation`，纯函数、客户端可算）。外圈每行补「本档实际」只读列，直接看得到该档在当前基数与抽数下值多少 cr（batch 下同时给单抽与 10 连两个数）。
  - 计价口径定下来：**内圈绝对 cr、外圈倍数 × 基数**。这样券价才是独立旋钮——奖池不动、把券价调高，返还率立刻往下走，面板当场反映。中途试过把内圈也改成「倍率 × 券价」（a7d27bf），结果两圈同步缩放、返还率被奖池形状钉死，券价 100/125/150/999 算出来都是 91.12%，定价这个旋钮等于废掉，故回退。
  - 需要让奖池跟着券价走时，改完券价点表单里的「按 ×N 重算内圈」按钮（相对**已保存那份**的比值等比放大内圈各行），是显式动作而非默认耦合。
  - `sanitizeInner` 两种写法都读：优先 `credits`，只给 `multiplier` 时按当次券价折回绝对 cr，避免那版配置留在库里读不出来。`updateLotterySettings` 的内圈校验随之回到 cr（±1,000,000）。
