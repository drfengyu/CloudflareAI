 ...
 * - 最终应用基础倍率（由管理员在 /admin/settings 设置）
 * - 1 credit = $1 USD (1:1)
 */
export async function calculateCredits(
  ...
  try {
    ...
    if (pricing.isImage || task === "Text-to-Image") {
      const credits = pricing.fixedPrice * baseMultiplier;
      console.log(`[calculateCredits] image: ${credits} cr (${pricing.fixedPrice} × ${baseMultiplier})`);
      return credits;
    }
